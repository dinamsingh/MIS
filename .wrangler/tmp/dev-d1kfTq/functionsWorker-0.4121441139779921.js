...
// .wrangler/tmp/pages-PMPjkT/functionsWorker-0.4121441139779921.mjs
...
var _getErrorMessage2 = /* @__PURE__ */ __name2((err) => {
  if (typeof err === "object" && err !== null) {
    const e = err;
...
var fetchWithAuth = /* @__PURE__ */ __name2((supabaseKey, supabaseUrl, getAccessToken, customFetch, tracePropagationOptions) => {
...
var TEMP_PASSWORD_LENGTH = 16;
function randomIndex(pool, crypto2) {
...
__name2(randomIndex, "randomIndex");
function shuffle(items, crypto2) {
  const result = [...items];
...
function generateTemporaryPassword(crypto2 = globalThis.crypto, length = TEMP_PASSWORD_LENGTH) {
  ...
  const guaranteed = [
    LOWER[randomIndex(LOWER, crypto2)],
    UPPER[randomIndex(UPPER, crypto2)],
    DIGITS[randomIndex(DIGITS, crypto2)],
    SYMBOLS[randomIndex(SYMBOLS, crypto2)]
  ];
  const rest = [];
  for (let i = guaranteed.length; i < effectiveLength; i += 1) {
    rest.push(ALL[randomIndex(ALL, crypto2)]);
  }
  return shuffle([...guaranteed, ...rest], crypto2).join("");
}
__name(generateTemporaryPassword, "generateTemporaryPassword");
__name2(generateTemporaryPassword, "generateTemporaryPassword");
...
__name(json, "json");
__name2(json, "json");
var onRequestOptions = /* @__PURE__ */ __name2(() => json({}, 204), "onRequestOptions");
var onRequestPost = /* @__PURE__ */ __name2(async (context) => {
  ...
  let warning;
  const { error: allowlistError } = await adminClient.from("allowed_teacher_emails").upsert({ email, added_by: null }, { onConflict: "email", ignoreDuplicates: true });
  if (allowlistError) {
    warning = "The Auth account was created, but adding the email to the allowlist failed. Add it manually.";
  }
  const { error: teacherUpsertError } = await adminClient.from("teachers").upsert(
    { id: userId, email, must_reset_password: true, onboarded: false },
    { onConflict: "id" }
  );
  if (teacherUpsertError) {
    warning = warning ? `${warning} Also, pre-creating the teacher profile failed.` : "The Auth account was created, but pre-creating the teacher profile failed.";
  }
...
__name(clampQuestionCount, "clampQuestionCount");
__name2(clampQuestionCount, "clampQuestionCount");
function normalizeDifficulty(value) {
  const v = (value ?? "").trim().toLowerCase();
  return v === "easy" || v === "hard" || v === "mixed" ? v : "mixed";
}
__name(normalizeDifficulty, "normalizeDifficulty");
__name2(normalizeDifficulty, "normalizeDifficulty");
var MAX_FIELD_LENGTH = 200;
function sanitizePromptField(value) {
  return value.replace(/[\r\n`]/g, " ").replace(/\s+/g, " ").trim().slice(0, MAX_FIELD_LENGTH);
}
__name(sanitizePromptField, "sanitizePromptField");
__name2(sanitizePromptField, "sanitizePromptField");
...
__name(difficultyInstruction, "difficultyInstruction");
__name2(difficultyInstruction, "difficultyInstruction");
function buildQuizPrompt(req) {
  ...
  const topicList = req.topics.map((t) => sanitizePromptField(t)).filter((t) => t.length > 0).map((t) => `- ${t}`).join("\n");
  return [
...
__name(buildQuizPrompt, "buildQuizPrompt");
__name2(buildQuizPrompt, "buildQuizPrompt");
function asRecord(value) {
  return typeof value === "object" && value !== null ? value : null;
}
__name(asRecord, "asRecord");
__name2(asRecord, "asRecord");
function parseOneQuestion(raw) {
  ...
  const correctIndex = typeof r.correctIndex === "number" && Number.isInteger(r.correctIndex) ? r.correctIndex : -1;
  if (correctIndex < 0 || correctIndex > 3) {
    return null;
  }
  const marks = typeof r.marks === "number" && Number.isFinite(r.marks) && r.marks >= 1 ? Math.floor(r.marks) : 1;
  return { text, options, correctIndex, marks };
}
__name(parseOneQuestion, "parseOneQuestion");
__name2(parseOneQuestion, "parseOneQuestion");
function parseGeneratedQuestions(value) {
  const record = asRecord(value);
  const rawList = Array.isArray(value) ? value : record && Array.isArray(record.questions) ? record.questions : [];
  const questions = [];
  let rejected = 0;
...
__name(parseGeneratedQuestions, "parseGeneratedQuestions");
__name2(parseGeneratedQuestions, "parseGeneratedQuestions");
...
__name(json2, "json2");
__name2(json2, "json");
var onRequestOptions2 = /* @__PURE__ */ __name2(() => json2({}, 204), "onRequestOptions");
var onRequestPost2 = /* @__PURE__ */ __name2(async (context) => {
  const { request, env } = context;
  if (!env.GEMINI_API_KEY) {
    return json2({ error: "AI is not configured on the server (missing GEMINI_API_KEY)." }, 503);
  }
  if (!env.SUPABASE_URL || !env.SUPABASE_ANON_KEY) {
    return json2({ error: "This server is not configured for quiz generation (missing Supabase env vars)." }, 503);
  }
  const authHeader = request.headers.get("authorization") ?? request.headers.get("Authorization") ?? "";
  const bearerMatch = authHeader.match(/^Bearer\s+(.+)$/i);
  const accessToken = bearerMatch?.[1]?.trim();
  if (!accessToken) {
    return json2({ error: "Not authorized." }, 403);
  }
  const callerClient = createClient(env.SUPABASE_URL, env.SUPABASE_ANON_KEY, {
    global: { headers: { Authorization: `Bearer ${accessToken}` } },
    auth: { persistSession: false, autoRefreshToken: false }
  ...
  if (unitName.length === 0 || topics.length === 0) {
    return json2({ error: "A unit with at least one topic is required to generate a quiz." }, 400);
  }
  ...
  try {
    ...
    return json2({ error: "Could not reach the AI service. Please try again." }, 502);
  }
  if (!aiResponse.ok) {
    const status = aiResponse.status === 429 ? 429 : 502;
    const message = aiResponse.status === 429 ? "AI rate limit reached. Please wait a moment and try again." : "The AI service returned an error. Please try again.";
  ...
  try {
    ...
    return json2({ error: "The AI service returned an unreadable response." }, 502);
  }
  const text = payload?.candidates?.[0]?.content?.parts?.[0]?.text ?? "";
  const parsedJson = extractJson(text);
  const { questions, rejected } = parseGeneratedQuestions(parsedJson);
  if (questions.length === 0) {
    return json2({ error: "The AI did not return usable questions. Please try again." }, 502);
  }
  return json2({ questions, rejected });
}, "onRequestPost");
var VALID_KINDS = ["theory", "lab", "project", "elective", "special"];
var MAX_NAME_LENGTH = 200;
var MAX_SUBJECTS = 15;
var MAX_UNITS_PER_SUBJECT = 12;
var MAX_TOPICS_PER_UNIT = 30;
var MAX_PDF_TEXT_CHARS = 4e4;
function buildSyllabusExtractionPrompt(semester, rawPdfText) {
...
__name(buildSyllabusExtractionPrompt, "buildSyllabusExtractionPrompt");
__name2(buildSyllabusExtractionPrompt, "buildSyllabusExtractionPrompt");
function asRecord2(value) {
  return typeof value === "object" && value !== null ? value : null;
}
__name(asRecord2, "asRecord2");
__name2(asRecord2, "asRecord");
...
__name2(cleanName, "cleanName");
function nullableCleanName(value) {
  if (value === null || value === void 0) {
    return null;
  }
  return cleanName(value);
}
__name(nullableCleanName, "nullableCleanName");
__name2(nullableCleanName, "nullableCleanName");
function parseOneUnit(raw, fallbackUnitNo) {
  ...
  const unitNo = typeof r.unitNo === "number" && Number.isInteger(r.unitNo) && r.unitNo > 0 ? r.unitNo : fallbackUnitNo;
  let rejected = 0;
...
__name(parseOneUnit, "parseOneUnit");
__name2(parseOneUnit, "parseOneUnit");
function parseOneSubject(raw) {
  ...
  const kind = VALID_KINDS.includes(kindRaw) ? kindRaw : "theory";
...
__name(parseOneSubject, "parseOneSubject");
__name2(parseOneSubject, "parseOneSubject");
function parseExtractedSyllabus(value) {
  const record = asRecord2(value);
  const rawList = Array.isArray(value) ? value : record && Array.isArray(record.subjects) ? record.subjects : [];
  const subjects = [];
  let rejected = 0;
...
__name(parseExtractedSyllabus, "parseExtractedSyllabus");
__name2(parseExtractedSyllabus, "parseExtractedSyllabus");
...
__name(json3, "json3");
__name2(json3, "json");
var onRequestOptions3 = /* @__PURE__ */ __name2(() => json3({}, 204), "onRequestOptions");
var onRequestPost3 = /* @__PURE__ */ __name2(async (context) => {
  const { request, env } = context;
  if (!env.GEMINI_API_KEY) {
    return json3({ error: "AI is not configured on the server (missing GEMINI_API_KEY)." }, 503);
  }
  if (!env.SUPABASE_URL || !env.SUPABASE_ANON_KEY) {
    return json3({ error: "This server is not configured for syllabus extraction (missing Supabase env vars)." }, 503);
  }
  const authHeader = request.headers.get("authorization") ?? request.headers.get("Authorization") ?? "";
  const bearerMatch = authHeader.match(/^Bearer\s+(.+)$/i);
  const accessToken = bearerMatch?.[1]?.trim();
  if (!accessToken) {
    return json3({ error: "Not authorized." }, 403);
  }
  const callerClient = createClient(env.SUPABASE_URL, env.SUPABASE_ANON_KEY, {
    global: { headers: { Authorization: `Bearer ${accessToken}` } },
    auth: { persistSession: false, autoRefreshToken: false }
  ...
  if (!Number.isInteger(semester) || semester < 1 || semester > 8) {
    return json3({ error: "A valid semester number (1-8) is required." }, 400);
  }
  if (pdfText.trim().length === 0) {
    return json3({ error: "No syllabus text was extracted from the PDF." }, 400);
  }
  const prompt = buildSyllabusExtractionPrompt(semester, pdfText);
  const endpoint = `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL2}:generateContent?key=${encodeURIComponent(env.GEMINI_API_KEY)}`;
  let aiResponse;
  try {
    aiResponse = await fetch(endpoint, {
      method: "POST",
    ...
    return json3({ error: "Could not reach the AI service. Please try again." }, 502);
  }
  if (!aiResponse.ok) {
    const status = aiResponse.status === 429 ? 429 : 502;
    const message = aiResponse.status === 429 ? "AI rate limit reached. Please wait a moment and try again." : "The AI service returned an error. Please try again.";
  ...
  try {
    ...
    return json3({ error: "The AI service returned an unreadable response." }, 502);
  }
  const text = payload?.candidates?.[0]?.content?.parts?.[0]?.text ?? "";
  const parsedJson = extractJson(text);
  const { subjects, rejected } = parseExtractedSyllabus(parsedJson);
  if (subjects.length === 0) {
    return json3({ error: "The AI could not identify any subjects in this document. Try a clearer PDF or check the semester number." }, 502);
  }
  return json3({ subjects, rejected });
}, "onRequestPost");
var routes = [
  {
...
// .wrangler/tmp/bundle-bCurNA/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bCurNA/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.4121441139779921.js.map
