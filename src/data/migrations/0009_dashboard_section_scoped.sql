-- ============================================================================
-- Migration: 0009_dashboard_section_scoped
-- Scope the dashboard to the SELECTED section, not the whole semester.
--
-- Why
-- ---
-- 0008 still aggregated every metric across all sections of the semester
-- (where sec.semester = p_semester), so the dashboard showed all 196 students
-- regardless of the section chosen in the global selector. The teacher expects
-- the dashboard to reflect exactly the section they selected.
--
-- This version: when p_section_id is provided, every metric (total students,
-- attendance %, internal marks, per-student metrics, attendance trend, syllabus
-- progress for that section's subjects) is scoped to that one section. When
-- p_section_id is null it falls back to the previous semester-wide behavior, so
-- nothing breaks for callers that omit it.
--
-- Signature is unchanged: get_dashboard_data(date, date, text, uuid).
-- ============================================================================
...
begin
    ...
    -- ---- Summary: total students in the selected section (or semester) ----
    select count(*)
    ...
    -- ---- Summary: average attendance % (scoped to the section when given) ----
    select coalesce(
    ...
    where (p_section_id is not null and a.section_id = p_section_id)
       or (p_section_id is null and (p_semester is null or sub.semester = p_semester));

    -- ---- Summary: average internal marks (scoped to the section when given) ----
    ...
    -- When a section is selected, scope to the subjects actually taught to that
    -- section (via its timetable); otherwise use the semester's subjects.
    select coalesce(
    ...
    where (p_section_id is not null and sub.id in (
    ...
    -- ---- Timetable entries for the active section (selected by id) ----
    ...
    -- ---- Per-student metrics (scoped to the selected section when given) ----
    select coalesce(jsonb_agg(
        jsonb_build_object(
            'studentId', st.id,
            'name', st.name,
    ...
    -- ---- Attendance trend (scoped to the section when given) ----
    select coalesce(jsonb_agg(
