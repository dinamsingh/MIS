-- ============================================================================
-- Migration: 0008_dashboard_rpc_by_id
-- Make get_dashboard_data() robust to section naming.
--
-- Why
-- ---
-- The previous version (0005) identified "all sections of a semester" by a
-- hardcoded name prefix match: sec.name LIKE 'CS-5%'. That breaks for any
-- section whose name is not exactly "CS-<n>..." — e.g. the real imported roster
-- uses "CSE-5A/5B/5C", so the dashboard counted 0 students and showed empty
-- cards even though the data exists.
--
-- This version removes all name matching:
--   * semester-level aggregation now filters by the real sections.semester
--     column (added in migration 0007), and
--   * the timetable section is selected directly by its id (p_section_id),
--     replacing the p_section letter + reconstructed name.
--
-- Frontend call becomes: get_dashboard_data(p_from_date, p_to_date,
--                                            p_semester, p_section_id).
-- ============================================================================

-- Drop every prior overload so only the id-based signature remains.
...
drop function if exists public.get_dashboard_data(date, date, text, uuid);
...
begin
    ...
    where p_semester is null or sec.semester = p_semester;
    ...
    from (
        ...
        where mv.internal_marks_snapshot is not null
          and (p_semester is null or sec.semester = p_semester)
